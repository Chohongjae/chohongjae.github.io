...
### 의문점
    jar 파일로 패키징된 파일을 java -jar로 실행하는 것이 아니라, 아래와 같이 kotlin 코드를 작성한 후
    코틀린 컴파일러로 컴파일한 .class 파일을 java 실행파일로 실행하려 하였다.
...
```
    ...
    하지만 java 실행파일로 kotlin class 파일은 실행되지 않고 아래와 같은 오류가 발생한다.
    kotlin 컴파일러가 java 컴파일러와 같이 동일한 class 파일을 만들어내어 java 실행파일로
    실행이 가능할 것이라고 생각했지만 오류가 발생한다.
...
{% raw %} <img src="https://chohongjae.github.io/assets/img/20210121코틀린이란무엇인가/kotlin2.png" alt=""> {% endraw %}
    ...
    하지만 args: Array<String> 를 지우니 java 실행파일로도 실행이 잘된다.    
